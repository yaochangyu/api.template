...
| 範本 | 用途 | 快速開始 |
|------|------|--------|
| [`basic-feature.template`](./assets/basic-feature.template) | 基礎的 .feature 檔案結構 | 複製 → 填入功能需求 → 調整情境 |
| [`api-test-setup.template`](./assets/api-test-setup.template) | API 測試框架與 WebApplicationFactory | 複製 → 調整命名空間與連線字串 |
| [`step-definitions.template`](./assets/step-definitions.template) | Reqnroll 步驟定義實作範例 | 複製 → 根據 .feature 調整步驟 |
...
**步驟**：
1. 複製 [`basic-feature.template`](./assets/basic-feature.template)
2. 填入功能名稱與背景條件
...
**步驟**：
1. 複製 [`api-test-setup.template`](./assets/api-test-setup.template)
2. 調整資料庫連線、DI 配置
