...
- **工作流程**：
  1. 需求分析
  2. 【互動】API 開發流程選擇（呼叫 `api-development-skill`）
  3. 【互動】測試策略選擇（呼叫 `bdd-testing-skill`）
  4. 撰寫 BDD 情境（如需要）
  5. 實作 Controller（呼叫 `api-development-skill`）
  6. 實作 Handler（呼叫 `handler-skill`）
  7. 實作 Repository（呼叫 `repository-design-skill`）
  8. 實作測試（呼叫 `bdd-testing-skill`）
  9. 執行測試驗證
- **使用的 Skills**：
...
- [ ] **步驟 1**：建立 `project-init-skill`
  - 為什麼：專案初始化是開發的第一步，必須優先完成
  - 產出：skill.md + references/project-initialization.md

- [ ] **步驟 2**：建立 `api-development-skill`
  - 為什麼：API 開發是核心流程，影響後續所有開發
  - 產出：skill.md + references + templates

- [ ] **步驟 3**：建立 `handler-skill`
  - 為什麼：Handler 是業務邏輯核心層
  - 產出：skill.md + references + template

- [ ] **步驟 4**：建立 `repository-design-skill`
  - 為什麼：Repository 設計影響資料存取架構
  - 產出：skill.md + references + templates

- [ ] **步驟 5**：建立 `error-handling-skill`
  - 為什麼：錯誤處理是跨層級的核心機制
  - 產出：skill.md + references + template

- [ ] **步驟 6**：建立 `bdd-testing-skill`
  - 為什麼：測試是品質保證的基礎
  - 產出：skill.md + references + templates

- [ ] **步驟 7**：建立 `middleware-skill`
  - 為什麼：中介軟體處理跨領域關注點
  - 產出：skill.md + references + template

- [ ] **步驟 8**：建立 `ef-core-skill`
