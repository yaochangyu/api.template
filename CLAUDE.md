...
1.  **主動提問，強制互動**：在執行任何有風險或需要使用者決策的操作前，你**必須**先向我提問，並等待我的確認。以結構化的清單（例如 1️⃣, 2️⃣, 3️⃣ 或 a, b, c）提供清晰的選項和說明，幫助我做出決定。
...
2. 【互動】詢問測試策略與範圍
   ...
   - 測試類型（BDD/單元測試/兩者）
   - 測試範圍與情境
3. 撰寫 BDD 情境 (.feature 檔案) - 如果需要 BDD 測試
4. 更新 OpenAPI 規格 (doc/openapi.yml)
5. 產生程式碼 (task codegen-api)
6. 實作 Handler 業務邏輯
